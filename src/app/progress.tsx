    ...
    <Screen contentStyle={styles.content}>
      ...
      <Card style={styles.card}>
        <View style={styles.row}>
          ...
          <View style={{ flex: 1, gap: 4 }}>
            <Display size={23}>{progress.levelName}</Display>
            <Body>
        ...
        <View
          ...
          accessibilityValue={{ min: 0, max: LEVEL_XP, now: progress.levelXp }}
          style={styles.track}
        >
          <View style={[styles.fill, { width: `${fillPercent}%` }]} />
        </View>
        <Body>
          {progress.nextLevelXp} XP to level {progress.level + 1}. Earn 20 XP for your first saved
          entry each day.
        </Body>
      </Card>
      ...
      <Card style={styles.card}>
        <Kicker color={colors.accent}>
          {progress.todayDone ? 'TODAY · COMPLETE' : 'TODAY’S CHECK-IN'}
        </Kicker>
        <Display size={21}>
          {progress.todayDone ? 'You made time for you.' : 'One moment worth remembering'}
        </Display>
        <Body>
          {progress.todayDone
            ? 'Your daily reward is earned. More writing is always optional.'
            : 'What is one small thing you want to remember about today?'}
        </Body>
        <PrimaryButton
          label={
            progress.todayDone
              ? 'Visit your journal'
              : 'Start today’s check-in · +20 XP'
          }
          onPress={() =>
            progress.todayDone
              ...
              : router.push({
                  pathname: '/write',
                  params: {
                    prompt: 'What is one small thing you want to remember about today?',
                  },
                })
          }
        />
      ...
      <View style={styles.section}>
        ...
        <View style={styles.row}>
          {([3, 5, 7] as const).map((goal) => (
            <Pressable
              ...
              accessibilityState={{ selected: settings.weeklyGoal === goal }}
              onPress={() => updateSettings({ weeklyGoal: goal })}
              style={[styles.goal, settings.weeklyGoal === goal && { backgroundColor: colors.accent }]}
            >
              <Text
                style={[
                  styles.goalText,
                  settings.weeklyGoal === goal && { color: colors.accentInk },
                ]}
              >
                {goal} days
              </Text>
        ...
        <View style={styles.row}>
          {Array.from({ length: settings.weeklyGoal }, (_, i) => (
            <View
              key={i}
              style={[styles.dot, i < progress.weeklyDays && { backgroundColor: colors.successBg }]}
            >
              <Icon
                name={i < progress.weeklyDays ? 'check' : 'leaf'}
                color={i < progress.weeklyDays ? colors.success : colors.faint}
                size={17}
              />
            </View>
          ))}
      ...
      <View style={styles.section}>
        <Kicker>
          MILESTONES · {progress.badges.filter((badge) => badge.earned).length}/
        ...
        <Body>
          Missed a day? Start again whenever you’re ready. Your XP stays with your saved journal
          days. Editing or adding extra entries never earns duplicate points.
        </Body>
      </View>
    </Screen>
  );
